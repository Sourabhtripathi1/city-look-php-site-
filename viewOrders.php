			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<table class="table table-hover table-bordered mx-2 my-5">
						...
						$res = mysqli_query($conn, "select * from user_orders");
						while ($or = mysqli_fetch_array($res)) {
							$Uid = $or["User_id"];
							$Orno = $or["Order_no"];


							$res_or=mysqli_query($conn, "SELECT * FROM orders WHERE User_id='$Uid' and Order_no='$Orno'");

							while($ord = mysqli_fetch_array($res_or)){
							// $itm = $ord['Item'];	
							// $x=mysqli_query()
						...
						?>
							<tr>
								...
								<td><?php echo $or['Order_no'] ?></td>
								<td><?php echo $or['User_id'] ?></td>
								
								<td><?php echo $ord['Item'] ?></td>
								<td><?php echo $ord['Price'] ?></td>
								<td><?php echo $ord['Quanity'] ?></td>

								<!-- <td> <?php //echo Arst($it) ?> </td> -->
								<!-- <td> <?php //echo  Arst($qt) ?> </td> -->
								<!-- <td> <?php //echo  Arst($prc) ?> </td> -->


								<td><?php echo $ord['Order_date'] ?></td>
								<td><?php echo $ord['Status'] ?></td>
								<td><?php echo $ord['Payment_method'] ?></td>
								<td><?php echo $ord['House_no'] ?></td>
								<td><?php echo $ord['PINCODE'] ?></td>
